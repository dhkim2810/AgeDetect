...
from util import AverageMeter, saliency_grid, saliency_bbox
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, monitors, args):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        outputs = t.tensor([])
        if args.dataloader.saliency == 'grid':
            r = np.random.rand(1)
            if r < args.dataloader.salgrid_prob:
                rand_index1 = t.randperm(inputs.size()[0]).cuda()
                rand_index2 = t.randperm(inputs.size()[0]).cuda()
                rand_index3 = t.randperm(inputs.size()[0]).cuda()
                # generate mixed sample
                target_a = targets
                target_b = targets[rand_index1]
                target_c = targets[rand_index2]
                target_d = targets[rand_index3]
                lam = 0.25
                new_inputs = t.clone(inputs.detach())
                for i, (idx1,idx2,idx3,idx4) in enumerate(zip(range(len(rand_index1)), rand_index1, rand_index2, rand_index3)):
                    salient_img = saliency_grid(inputs[idx1],inputs[idx2],inputs[idx3],inputs[idx4], original=args.dataloader.original)
                    new_inputs[i] = t.tensor(salient_img).cuda()
                # compute output
                model.zero_grad()
                outputs = model(new_inputs)
                loss = criterion(outputs, target_a)*lam + criterion(outputs, target_b)*lam + criterion(outputs, target_c)*lam + criterion(outputs, target_d)*lam
            else:
                model.zero_grad()
                outputs = model(inputs)
                loss = criterion(outputs, targets)
        elif args.dataloader.saliency == 'original':
            r = np.random.rand(1)
            beta = args.dataloader.beta
            if beta > 0 and r < args.dataloader.salgrid_prob:
                # generate mixed sample
                rand_index = t.randperm(inputs.size()[0]).cuda()
                target_a = targets
                target_b = targets[rand_index]
                lam = 0.0
                new_inputs = t.clone(inputs.detach())
                for i in range(len(rand_index)):
                    bbx1, bby1, bbx2, bby2 = saliency_bbox(inputs[rand_index[i]], np.random.beta(beta, beta), True, True)
                    new_inputs[i, :, bbx1:bbx2, bby1:bby2] = inputs[rand_index[i], :, bbx1:bbx2, bby1:bby2]
