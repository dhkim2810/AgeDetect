...
class FacialDataset(Dataset):
    def __init__(self, data_path, transforms, random_discretize=False, M=30, N=10, max_age=120):
        ...
        self.random_discretize = random_discretize
        print("Loading Images..")
        for pth in tqdm(self.age_path):
            label = int(pth.split('_')[0].split('/')[-1])
            self.labels.append(label)
            self.imgs.append(pth)
        if random_discretize:
            self.discretized_labels = torch.zeros(len(self.age_path), M,N)
            self.centroids,_ = torch.sort(torch.randint(1,max_age,(M,N)),dim=1)
            for i,label in enumerate(self.labels):
                _, indices = torch.min(abs(label-self.centroids), dim=1)
                self.discretized_labels[i,:,:] = (indices.reshape(M,1) == torch.arange(N).reshape(1,N)).float()
            
    def __getitem__(self, idx):
        image = self.transform(Image.open(self.imgs[idx]))
        if self.random_discretize:
            label = self.discretized_labels[idx]
        else:
...
def get_data_loader(config, data_path, batch_size, num_workers,train_val_ratio):
    ...
    full_dataset = FacialDataset(data_path, val_transform, True)
    train_size = int(train_val_ratio * len(full_dataset))
    val_size = len(full_dataset) - train_size
    train_dataset, val_dataset = torch.utils.data.random_split(full_dataset, [train_size, val_size])
    train_dataset.dataset = FacialDataset(data_path, train_transform, True)
